...
def train(args, model, graph, labels, train_idx, val_idx, test_idx, optimizer, evaluator, order_attn):
    model.train()
...
def run(args, graph, labels, train_idx, val_idx, test_idx, evaluator, n_running, order_attn):
    ...
    for epoch in range(1, args.n_epochs + 1):
        ...
        acc, loss = train(args, model, graph, labels, train_idx, val_idx, test_idx, optimizer, evaluator_wrapper, order_attn)
    ...
    torch.save(final_pred, "./outputs/hla_gat_kd/best_pred_run%i.pkl" % n_running)
    ...
    return best_val_acc, final_test_acc, me_test_acc, val_epoch, test_epoch, best_val_loss, me_val_loss, me_val_acc
...
def main():
    ...
    argparser.add_argument("--cpu", type=bool, default=False, help="CPU mode. This option overrides --gpu.")
    ...
    argparser.add_argument("--use_norm", type=bool, default=True, help="Use symmetrically normalized adjacency matrix.")
    argparser.add_argument("--use_labels", type=bool, default=True, help="Use labels in the training set as input features.")
    argparser.add_argument("--n_label_iters", type=int, default=1, help="number of label iterations")  # CS:1
    argparser.add_argument("--save_pred", type=bool, default=False, help="save final predictions")  # CS:True

    argparser.add_argument("--mask-rate", type=float, default=0.5, help="mask rate")
    argparser.add_argument("--no_attn_dst", type=bool, default=True, help="Don't use attn_dst.")
    ...
    argparser.add_argument("--input-drop", type=float, default=0.25, help="input drop rate")  # l reuse/CS:0.25 else:0.1
    argparser.add_argument("--attn-drop", type=float, default=0.0, help="attention drop rate")
    argparser.add_argument("--edge-drop", type=float, default=0.3, help="edge drop rate")  # label reuse/CS:0.3 else:0.1
    ...
    argparser.add_argument('--patience', type=int, default=100, help='Patience')
    ...
    if args.order_attn:  # me
        ...
        order_attn = torch.tensor([01., 0.2])
    ...
    val_accs, me_val_accs, test_accs = [], [], []

    for i in range(args.n_runs):
        seed(args.seed + i)
        val_acc, test_acc, me_test_acc, val_epoch, test_epoch, best_val_loss, me_val_loss, me_val_acc = \
            run(args, graph, labels, train_idx, val_idx, test_idx, evaluator, i + 1, order_attn)
